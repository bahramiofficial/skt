            ...
            <div class="col-12">
                ...
                <form action="{{ route('categoryshop.update', $categoryshop->id) }}" class="padding-30" method="post">
                    ...
                    <select class="select" name="category_id" id="category_id">
                        ...
                        @foreach($parentCategoryshops as $parentCategoryshop)
                          <option value="{{ $parentCategoryshop->id }}" @if($parentCategoryshop->id === $categoryshop->parent_id) selected @endif>{{ $parentCategoryshop->name }}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                      <p class="error">{{ $message }}</p>
