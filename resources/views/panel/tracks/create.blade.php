<x-panel-layout>
  ...
  <div class="breadcrumb">
      <ul>
          ...
          <li><a href="{{ route('track.create') }}" class="is-active">ثبت پیست  جدید</a></li>
      </ul>
    </div>
    <div class="main-content font-size-13">
        <div class="row no-gutters  bg-white">
            <div class="col-12">
                <p class="box__title">ثبت پیست</p>


                {{-- {{ route('tracks.store') }} --}}
                <form action="{{ route('track.store') }}" class="padding-30" method="post" enctype="multipart/form-data" >
                    @csrf
                    <input type="text" name="name" class="text" placeholder="نام پیست" value="{{ old('name') }}">
                    @error('name')
                      <p class="error">{{ $message }}</p>
                    @enderror

                    <input type="text" name="lat" class="text" placeholder="lat" value="{{ old('lat') }}">
                    @error('lat')
                      <p class="error">{{ $message }}</p>
                    @enderror

                    <input type="text" name="long" class="text" placeholder="long" value="{{ old('long') }}">
                    @error('long')
                      <p class="error">{{ $message }}</p>
                    @enderror
